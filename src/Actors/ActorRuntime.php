...
use Rabbit\Dapr\Actors\Internal\Caches\CacheInterface;
use Rabbit\Dapr\Actors\Internal\Caches\FileCache;
use Rabbit\Dapr\Client\DaprClient;
use Rabbit\Dapr\exceptions\DaprException;
use Rabbit\Dapr\exceptions\Http\NotFound;
use Rabbit\Dapr\exceptions\SaveStateFailure;
use Exception;
use Rabbit\Base\App;
use ReflectionClass;
...
class ActorRuntime
{
    ...
    {
        if (
            $reflection->implementsInterface('Rabbit\Dapr\Actors\IActor')
    ...
    protected function get_actor(ReflectionClass $reflection, ActorReference $reference, array $states): IActor
    {
        ...
        if (!App::$di->container->has(ActorReference::class)) {
            App::$di->container->set(ActorReference::class, $reference);
        }
        $actor = create($reflection->getName(), $states, singleTon: false);
